...
done_files_SRnd = [file.split('.')[0] for file in os.listdir('./scrambled_maps/LL/SRand_timescaleChanges_data') if file.endswith('.npy')]
...
for i in range(len(files_SFix)):
    ...
    for perm_i in range(PERMS + 1):
        ...
        #print(timescales_SFix[repeat])
        #print(timescales_SRnd[repeat])
    np.save(f'./scrambled_maps/LL/SFix_timescaleChanges_data/{area_SFix}.npy', timescales_SFix)
    np.save(f'./scrambled_maps/LL/SRand_timescaleChanges_data/{area_SRnd}.npy', timescales_SRnd)
